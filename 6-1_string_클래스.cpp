...
//간단한 예제
/*int main() {
    // 표준이므로 std안에 string이 정의되어 있다
    std::string s = "abc";
    std::string t = "def";
    std::string s2 = s;

    std::cout << s << " 의 길이 : " << s.length() << std::endl;
    std::cout << " 뒤에 " << t << "를 붙이면 : " << s + t << std::endl;
...
public:
    ...
    void add_employee(Employee* employee) {
        // 사실 current_employee 보다 alloc_employee가 가 더
        //많아지는 경우 반드시 재할당을 해야하나
        //여기서는 항상 할당된 크기가 현재 총 직원수 보다 많다고 정의
